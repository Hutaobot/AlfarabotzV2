...
if (command == 'megumin') {
let res = await (await fetch(`https://raw.githubusercontent.com/KazukoGans/database/main/anime/megumin.json`)).json()
let cita = res[Math.floor(Math.random() * res.length)]
await conn.sendButtonImg(m.chat, cita, `Nihh ${command} nya @${m.sender.split('@')[0]}`, wm, `Next`, `${usedPrefix}${command}`, m, {mentions: [m.sender], jpegThumbnail: await(await fetch(cita)).buffer()})
}
if (command == 'mikasa') {
...
if (command == 'yuri') {
let res = await (await fetch(`https://raw.githubusercontent.com/KazukoGans/database/main/anime/yuri.json`)).json()
let cita = res[Math.floor(Math.random() * res.length)]
await conn.sendButtonImg(m.chat, cita, `Nihh ${command} nya @${m.sender.split('@')[0]}`, wm, `Next`, `${usedPrefix}${command}`, m, {mentions: [m.sender], jpegThumbnail: await(await fetch(cita)).buffer()})
}
if (command == 'shinobu') {
